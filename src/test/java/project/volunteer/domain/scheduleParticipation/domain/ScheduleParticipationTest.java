...
class ScheduleParticipationTest {
    ...
    private final RecruitmentParticipation recruitmentParticipation = new RecruitmentParticipation(recruitment, user,
            ParticipantState.JOIN_APPROVAL);
    private final Schedule schedule = new Schedule(timetable, "test", "unicef", address, 10, IsDeleted.N, 0,
            recruitment);


    @ParameterizedTest
    @EnumSource(names = {"PARTICIPATING", "PARTICIPATION_CANCEL"})
    @DisplayName("PARTICIPATION_CANCEL_APPROVAL 상태만 재 일정 참여가 가능하다.")
    void checkReParticipation(ParticipantState invalidState) {
        //given
        ScheduleParticipation scheduleParticipation = new ScheduleParticipation(schedule, recruitmentParticipation,
                invalidState);

        //when
        Boolean result = scheduleParticipation.canReParticipation();
